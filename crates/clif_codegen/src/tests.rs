...
unsafe fn run_code<I, O>(input: I) -> Result<O, String> {
    // Pass the string to the JIT, and it returns a raw pointer to machine code.
    let code_ptr = compile_adder_printer()?;
...
#[test]
fn adder() {
    let result: Result<(), String> = unsafe { run_code((20_i64, 3_i64)) };
