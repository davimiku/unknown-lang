...
use crate::type_expr::{
    CallExpr, RecordTypeExpr, TypeExpr, TypeRefExpr, TypeSymbol, UnionTypeExpr,
};
use crate::typecheck::{check_expr, infer_expr, infer_module, CoreTypes, TypeDatabase, TypeResult};
...
impl Context {
    ...
    fn lower_pattern(&mut self, ast: ast::Pattern, parent: Option<Idx<Pattern>>) -> Idx<Pattern> {
        ...
        match ast {
            ...
            ast::Pattern::Wildcard(_syntax_node) => {
    ...
    fn lower_path(&mut self, path: ast::PathExpr) -> Expr {
        ...
        // or is Color more of a "synthesized record"?
    ...
    fn lower_index_expr(&mut self, subject: Expr, index: Expr) -> Expr {
        todo!("may not implement index expressions")
    }
...
impl Context {
    fn lower_type_expr(
        ...
        let type_expr = if let Some(ast) = ast {
            ...
            let type_expr = match ast {
                ...
                TE::Union(ast) => self.lower_union_type(ast, name),
                TE::Record(ast) => self.lower_record_type(ast, name),
            };
    ...
    fn lower_union_type(&mut self, ast: ast::Union, name: Option<TypeSymbol>) -> TypeExpr {
        let variants = ast
            .variants()
            .iter()
            .map(|item| {
    ...
    fn lower_record_type(&mut self, ast: ast::Record, name: Option<TypeSymbol>) -> TypeExpr {
        ...
        TypeExpr::Record(RecordTypeExpr { name, fields })
    }
