...
use std::rc::Rc;
use vm_boxed_types::Boxed;
use vm_codegen::{FunctionChunk, InvalidOpError, Op, Readable, COMPILER_BRAND};
...
/// Character appended to names/keys to indicate it was modified
/// or generated by the compiler
// pub(crate) const COMPILER_BRAND: char = '~';

pub fn run(chunk: FunctionChunk) -> InterpretResult<()> {
...
pub fn run_and_return<T>(chunk: FunctionChunk) -> InterpretResult<Vec<Word>>
where
    T: FromWordVec,
{
    let mut vm = VM::new(chunk);
...
pub struct VM {
    ...
    /// Instruction Pointer
    // TODO: review what the book says about a pointer being faster,
    // needs unsafe/pointer arithmetic? Ideally this ip is kept in
    // a CPU register, not sure how to check this
    ip: usize,
...
impl VM {
    pub(crate) fn new(main_chunk: FunctionChunk) -> Self {
        VM {
            chunk: main_chunk,
            ..Default::default()
        }
    ...
    fn alloc_function(&mut self, chunk: FunctionChunk, name: Option<&str>) {
        let name = name.map(|n| self.interner.get_or_intern(n));
        let boxed = Boxed::new_function(chunk, 0, name);
