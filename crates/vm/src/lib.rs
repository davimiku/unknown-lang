...
use vm_codegen::{Chunk, InvalidOpError, Op, Readable, COMPILER_BRAND};
...
pub fn run_and_return<T>(chunk: Chunk) -> InterpretResult<Vec<Word>>
where
    T: FromWordVec,
{
    let mut vm = VM::new(chunk);
...
impl VM {
    pub(crate) fn new(main_chunk: Chunk) -> Self {
        VM {
            chunk: main_chunk,
            ..Default::default()
        }
    ...
    fn alloc_function(&mut self, chunk: Chunk, name: Option<&str>) {
