...
pub(crate) type BranchTarget = (i64, BlockTarget);
...
pub struct Place {
    pub local: Idx<Local>,
    pub projections: Box<[ProjectionElem]>,
}
...
pub enum ProjectionElem {
    Deref,
    /// A field (e.g., f in _1.f) is a field of a record. It is identified by
    /// the numerical index of the field within the record type.
    // TODO - Read more about projections in the rustc-dev-guide
    Field(FieldIdx, hir::Type),
    ...
    /// Get the "payload" of a variant of a union
    ///
    /// The included Key is the name of the variant, used for printing MIR.
    DowncastVariant(Option<Key>, VariantIdx),

    /// Create an instance of the union type by prepending the variant's tag
    /// to the data
    ///
    /// The included Key is the name of the variant, used for printing MIR
